...
function IconMobile() {
  return (
    <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      <rect x="3" y="14" width="7" height="7" rx="0.5" /><rect x="14" y="14" width="3" height="3" rx="0.5" />
...
export default function DevPage() {
  return (
    <main className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center gap-12 p-8">
      ...
      >
        <p className="text-xs font-mono text-amber-400 uppercase tracking-[0.3em] mb-3">◆ Dev — Sistema de Turnos</p>
        <h1 className="text-4xl font-black uppercase tracking-tight text-zinc-100">Módulos</h1>
      ...
      <nav className="grid grid-cols-2 gap-3 w-full max-w-sm">
        {screens.map(({ href, label, sub, active, Icon }, i) => {
          ...
          if (!active) return (
            <div key={label} aria-disabled="true" className={`${base} opacity-20 cursor-not-allowed select-none`}>
          ...
          return (
            <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07, duration: 0.35, ease: [0.16, 1, 0.3, 1] }}>
              <motion.div whileHover={{ borderColor: "#fbbf24" }} whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 400, damping: 28 }} style={{ borderColor: "#27272a" }} className="border">
                <Link href={href} className={`${base} group hover:bg-zinc-900 transition-colors duration-150 block`}>
                  <span className="text-zinc-600 group-hover:text-amber-400 transition-colors duration-150"><Icon /></span>
                  <div className="text-center">
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-xs font-mono text-zinc-700 tracking-widest">
